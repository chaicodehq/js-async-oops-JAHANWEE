...
export async function checkSeatAvailability(trainNumber, date, classType) {
  ...
  if (!/^\d{5}$/.test(trainNumber)) throw new Error('Invalid train number! 5 digit hona chahiye.');
  ...
  return { trainNumber, date, classType, available: seats > 0, seats, waitlist: Math.floor(Math.random() * 21) };
}
...
export async function bookTicket(passenger, trainNumber, date, classType) {
  ...
  if (availability.available) {
    return { pnr: 'PNR' + Math.floor(Math.random() * 1000000), passenger, trainNumber, date, class: classType, status: 'confirmed', fare: FARES[classType] };
  }
  return { pnr: 'PNR' + Math.floor(Math.random() * 1000000), passenger, trainNumber, date, class: classType, status: 'waitlisted', waitlistNumber: Math.floor(Math.random() * 20) + 1 };
}

export async function cancelTicket(pnr) {
  await new Promise(r => setTimeout(r, 50));
  if (!pnr || !String(pnr).startsWith('PNR')) throw new Error('Invalid PNR number!');
  return { pnr, status: 'cancelled', refund: Math.floor(Math.random() * 901) + 100 };
}
...
export async function getBookingStatus(pnr) {
  ...
  return { pnr, status: statuses[Math.floor(Math.random() * statuses.length)], lastUpdated: new Date().toISOString() };
}
...
export async function raceBooking(trainNumbers, passenger, date, classType) {
  const bookings = trainNumbers.map(tn => bookTicket(passenger, tn, date, classType));
  try {
    return await Promise.any(bookings);
  } catch {
