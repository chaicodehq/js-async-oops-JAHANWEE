...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    if (this.#residents.find(r => r.name === name)) return { success: false, message: 'Already a resident!' };
  ...
  verifyAccess(name) {
    const found = this.#residents.find(r => r.name === name);
    const entry = { name, time: new Date().toISOString(), allowed: !!found };
    this.#accessLog.push(entry);
    if (found) return { allowed: true, message: `Swagat hai ${name}!` };
  ...
  isResident(name) {
    return !!this.#residents.find(r => r.name === name);
  }
}
