...
export class TempleQueue {
  ...
  get length() { return this.#devotees.length; }
  get isEmpty() { return this.#devotees.length === 0; }
  get vipEnabled() { return this.#vipEnabled; }

  set vipEnabled(value) {
    if (typeof value !== 'boolean') throw new TypeError('VIP status must be a boolean');
  ...
  static merge(queue1, queue2) {
    ...
    const merged = new TempleQueue(`${queue1.templeName}-${queue2.templeName}`, arr1.length + arr2.length + 10);
  ...
  static fromArray(templeName, maxCapacity, arr) {
    ...
    arr.forEach(name => queue.enqueue(name, 'regular'));
    return queue;
  }

  *[Symbol.iterator]() {
    for (const devotee of this.#devotees) yield devotee;
  }
}
