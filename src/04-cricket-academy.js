...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    return this.innings.reduce((sum, i) => sum + i.strikeRate, 0) / this.innings.length;
  }

  getProfile() {
    return { ...super.getProfile(), battingStyle: this.battingStyle, role: 'batsman', totalRuns: this.innings.reduce((s, i) => s + i.runs, 0), inningsPlayed: this.innings.length };
  }
}
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    return this.spells.reduce((sum, s) => sum + s.economy, 0) / this.spells.length;
  }

  getProfile() {
    return { ...super.getProfile(), bowlingStyle: this.bowlingStyle, role: 'bowler', totalWickets: this.spells.reduce((s, sp) => s + sp.wickets, 0), spellsBowled: this.spells.length };
  }
}
...
export class AllRounder extends Player {
  ...
  getProfile() {
    return { ...super.getProfile(), battingStyle: this.battingStyle, bowlingStyle: this.bowlingStyle, role: 'allrounder', totalRuns: this.innings.reduce((s, i) => s + i.runs, 0), totalWickets: this.spells.reduce((s, sp) => s + sp.wickets, 0), inningsPlayed: this.innings.length, spellsBowled: this.spells.length };
  }
}
