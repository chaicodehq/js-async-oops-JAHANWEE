...
const FEEDBACKS = ['Bahut badhiya!', 'Accha hai, improve karo', 'Investors impressed!'];

export class Startup {
  #funding;

  constructor(name, founder, domain) {
    if (!VALID_DOMAINS.includes(domain)) throw new Error('Invalid domain! Choose from: fintech, edtech, healthtech, foodtech');
    this.name = name;
    this.founder = founder;
    this.domain = domain;
    this.#funding = 0;
...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    if (this.#startups.find(s => s.startup.name === startup.name)) throw new Error('Startup already admitted!');
    if (this.#startups.length >= this.maxStartups) throw new Error('Incubator full!');
  ...
  async assignMentor(startupName, mentor) {
    await new Promise(r => setTimeout(r, 10));
    if (!this.#startups.find(s => s.startup.name === startupName)) throw new Error('Startup not found!');
  ...
  async conductDemo(startupName) {
    ...
    return { startup: startupName, score: Math.floor(Math.random() * 41) + 60, feedback: FEEDBACKS[Math.floor(Math.random() * FEEDBACKS.length)], timestamp: new Date().toISOString() };
  }
  ...
  getStartupsByDomain(domain) {
    return this.#startups.filter(s => s.startup.domain === domain).map(s => s.startup);
  }

  getTopFunded(n) {
    if (n <= 0) return [];
    return this.#startups.map(s => s.startup).sort((a, b) => b.funding - a.funding).slice(0, n);
  }

  *[Symbol.iterator]() {
    for (const entry of this.#startups) yield entry.startup;
  }
...
export async function runDemoDay(incubator) {
  ...
  const results = await Promise.allSettled(startups.map(s => incubator.conductDemo(s.name)));
  return { incubator: incubator.name, totalStartups: startups.length, results: results.map(r => r.status === 'fulfilled' ? r.value : r.reason), timestamp: new Date().toISOString() };
}
