...
export function placeOrder(restaurant, items) {
  return new Promise((resolve, reject) => {
    if (!restaurant || !items || items.length === 0) return reject(new Error('Invalid order details!'));
    setTimeout(() => resolve({ orderId: Math.floor(Math.random() * 10000), restaurant, items, status: 'placed', timestamp: new Date().toISOString() }), 50);
  });
}

export function confirmOrder(order) {
  return new Promise((resolve, reject) => {
    if (!order || !order.orderId || order.status !== 'placed') return reject(new Error('Order cannot be confirmed!'));
...
export function assignRider(order) {
  return new Promise((resolve, reject) => {
    if (!order || order.status !== 'confirmed') return reject(new Error('Order not confirmed yet!'));
...
export function deliverOrder(order) {
  return new Promise((resolve, reject) => {
    if (!order || !order.rider) return reject(new Error('No rider assigned!'));
...
export function processMultipleOrders(orderList) {
  return Promise.allSettled(orderList.map(o => processDelivery(o.restaurant, o.items)));
}
