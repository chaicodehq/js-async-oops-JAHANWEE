...
LassiStand.prototype.addFlavor = function(flavor, price) {
  if (price <= 0) return -1;
  if (this.menu.find(f => f.flavor === flavor)) return -1;
...
LassiStand.prototype.takeOrder = function(customerName, flavor, quantity) {
  ...
  const order = { id: this._nextOrderId++, customer: customerName, flavor, quantity, total: item.price * quantity, status: 'pending' };
